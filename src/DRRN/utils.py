...
def draw_zoom(data1, data2):
    ...
    y_axis1 = data1
    y_axis2 = data2
    ax.plot(x_axis, y_axis1, c='red', label='Target')
    ax.plot(x_axis, y_axis2, c='green', linestyle='--', label='Super-Resolution')
    plt.legend(loc='best')
    ax.set_ylabel('强度(a.u.)')
    ax.set_xlabel('波长(nm)')
    ax_inset = ax.inset_axes((0.4, 0.3, 0.5, 0.4))
    ax_inset.plot(x_axis, y_axis1, c='red')
    ax_inset.plot(x_axis, y_axis2, c='green', linestyle='--')
    zone_left = 0
    zone_right = 500
    x_ratio = 0
    y_ratio = 0
    x0 = x_axis[zone_left] - (x_axis[zone_right] - x_axis[zone_left]) * x_ratio
    x1 = x_axis[zone_right] - (x_axis[zone_right] - x_axis[zone_left]) * x_ratio
    y = np.hstack((y_axis1[zone_left:zone_right], y_axis2[zone_left:zone_right]))
    y0 = np.min(y) - (np.max(y) - np.min(y)) * y_ratio
    y1 = np.max(y) - (np.max(y) - np.min(y)) * y_ratio
